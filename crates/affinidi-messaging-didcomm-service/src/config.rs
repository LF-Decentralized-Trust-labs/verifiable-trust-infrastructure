...
pub struct ListenerConfig {
    ...
    /// Optional TDK configuration for the listener's internal shared state.
    /// When set, the listener uses this config (e.g., network-mode DID resolver)
    /// instead of the default local-mode resolver. Critical for TEE enclaves
    /// where DID resolution must go through a sidecar.
